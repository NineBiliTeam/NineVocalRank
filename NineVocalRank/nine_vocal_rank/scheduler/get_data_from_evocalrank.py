...
async def get_data_from_evocalrank():
    ...
    周刊对不起
    """
    result = (await (await _http.get_async_session()).get("https://www.evocalrank.com/data/info/latest.json")).json()
    keywords = ["main_rank", "oth_pickup", "super_hit", "pick_up", "Vocaloid_pick_up", "history-1-year", "history-10-year", "ed", "op"]
