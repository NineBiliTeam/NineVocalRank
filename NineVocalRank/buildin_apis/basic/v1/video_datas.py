...
@video_router.get("/GetStat")
async def get_stat(video_id: Annotated[VideoIDRequestModel, Query()]) -> Video:
    """
    获取视频的当前数据.
    此处获取的Video对象为NineBiliRank处理视频的对象
    """
    video = Video(video_id=video_id.vid)
    await video.async_update_basic_data()
    return video


@video_router.get("/SearchFromDB")
